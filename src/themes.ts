export const THEMES: Record<string, string> = {
  ...
  luca: 'bold:magenta:claude:model-letter sep:pipe cyan:nerd:repo cyan:git:repo sep:none text:: sep:none green:git:branch if:subdir(sep:none white:text:/ sep:none white:fs:relative) if:dirty(sep:pipe git:dirty) sep:pipe white:account:email sep:newline bold:white:usage:5h-bar:8 usage:5h (usage:5h-pace-emoji sep:none usage:5h-pace) dim:usage:5h-reset sep:pipe bold:white:usage:week-bar:8 usage:week (usage:week-pace-emoji sep:none usage:week-pace) dim:usage:week-reset sep:pipe white:cost:total',
};
